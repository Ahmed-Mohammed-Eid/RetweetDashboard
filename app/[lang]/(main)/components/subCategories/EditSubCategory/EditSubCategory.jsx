...
export default function EditSubCategory({ subCategoryId, lang }) {
    ...
    function getSubCategory(id) {
        ...
        axios.get(`${process.env.API_URL}/sub/category?subCategoryId=${id}`, {
            ...
            .then(res => {
                ...
                // LOOP THROUGH THE ITEMS AND ITEMS EN AND FORMATE THEM TO BE [{nameAR: '', nameEN: ''}]
                const itemsArray = [];
                for (let i = 0; i < subCategory?.subCategory?.items?.length; i++) {
                    itemsArray.push({
                        nameAR: subCategory?.subCategory?.items[i],
                        nameEN: subCategory?.subCategory?.itemsEn[i]
                    });
                }

                // SET THE ITEMS
                setItems({
                    itemsNumber: subCategory?.subCategory?.items?.length || 0,
                    items: itemsArray
                });
    ...
    function editSubCategory(event) {
        ...
        // VALIDATE THE FORM
        if (!form.subCategoryName || !form.subCategoryNameEn || !form.mainCategoryId ) {
        ...
        >
            <div className={'card mb-2'}>
                <h1 className={'text-2xl font-bold mb-4 uppercase'}>
                    {lang === 'en' ? 'Edit Sub Category' : 'تعديل الفئة الفرعية'}
                </h1>
                <div className="grid formgrid p-fluid">
                    ...
                    <div className="col-12 mb-2 lg:mb-2" dir={'ltr'}>
                        <label className={'mb-2 block'} htmlFor="male-image"
                               dir={lang === 'ar' ? 'rtl' : 'ltr'}>Files</label>
                        <CustomFileUpload
                            setFiles={(files) => {
                                setForm({ ...form, files });
                            }}
